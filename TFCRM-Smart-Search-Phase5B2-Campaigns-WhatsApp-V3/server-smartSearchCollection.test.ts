...
import {
  normalizedCollectionContainsAll,
  smartFilterCollection,
} from "../../client/src/lib/smartSearchCollection";

// SMART_SEARCH_PHASE5B2_V3_SERVER_COLLECTION_TEST
const rows = [
...
describe("Smart Search Phase 5B2 collection fallback", () => {
  ...
  it("does not fuzzy-match numeric-only identifiers", () => {
    ...
    expect(
      smartFilterCollection(numericRows, "01012345679", (row) => row.text),
    ).toEqual([]);
  });
});
